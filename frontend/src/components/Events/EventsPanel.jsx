...
export default function EventsPanel() {
  ...
  return (
    <div className="space-y-4">
      ...
      <EventList title="Upcoming" events={upcoming} onDelete={(id) => deleteMutation.mutate(id)} />
      {past.length > 0 && (
        <EventList title="Past" events={past} onDelete={(id) => deleteMutation.mutate(id)} muted />
      )}
    </div>
  )
}

function EventList({ title, events, onDelete, muted }) {
    ...
    <div>
      ...
      <div className="space-y-2">
        {events.map((ev) => (
          <div
            key={ev.id}
            className={`flex items-center gap-3 bg-white rounded-lg border border-gray-200 px-3 py-2.5 group ${muted ? 'opacity-60' : ''}`}
          >
            <span
